...
| `randomchoice` | 历史/服务端字段；当前没有完整的创建、读取和验证闭环，不应据此宣称已支持随机化 |
...
| 矩阵 | `矩阵单选`, `矩阵多选`, `矩阵量表`, `矩阵填空`, `矩阵滑动条` |
| 表格 | `表格数值`, `表格填空`, `表格下拉框`, `表格组合`, `自增表格` |
...
`矩阵数值题` 可以在读取既有问卷时通过 `q_type=7, q_subtype=706` 识别，但当前问卷星 JSONL 创建接口会明确拒绝该 qtype。需要此题型时转到 Web 编辑器；CLI/MCP 会在本地预检阶段直接提示，不会发出创建请求或自动降级成其他题型。
...
| 调研模型 | `情景随机`, `BWS`, `MaxDiff`, `图片PK`, `联合分析`, `Kano模型`, `SUS模型`, `品牌漏斗`, `货架题`, `BPTO模型`, `PSM模型`, `价格断裂点`, `层次分析`, `选项分类`, `CATI调研`, `文字点睛`, `心理学实验`, `循环评价`, `热力图` |
| 预设信息 | `姓名`, `基本信息`, `身份证号`, `国家及地区`, `省市`, `省市区`, `邮箱`, `手机`, `高校`, `邮寄地址`, `社会阶层`, `企业信息` |
| 系统采集 | `设备信息`, `城市级别`, `当前语言`, `答题录音`, `答卷摄像`, `分页计时器` |
...
这三类专业模型必须同时提供至少 3 个非空 `mdattr`，以及正整数 `pertaskcount`（每个任务抽取的对象数）和 `tasklength`（任务数）。`图片PK` 的 `mdattr` 每一项还必须是上传接口返回的图片地址；不能把本地文件路径或普通文字当作图片地址。

```jsonl
{"qtype":"MaxDiff","title":"请选择最喜欢和最不喜欢的对象","mdattr":["对象 A","对象 B","对象 C","对象 D"],"pertaskcount":2,"tasklength":2}
```
...
`survey get`、`response submit-template` 等输出中仍可能出现 `q_type`、`q_subtype` 和 `q_index`。这些字段用于解释服务端结果和构造答卷，不要复制回 JSONL。使用 `create` 创建后，题号由服务端分配，可能从 1、2 或其他值开始；提交答卷时始终以 `wjx response submit-template --vid <vid>` 返回的 `q_index` 为准。
