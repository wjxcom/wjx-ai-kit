 ...
 */
export function parsedQuestionsToWire(questions) {
    const unsupported = [];
    ...
    if (unsupported.length > 0) {
        ...
        throw new Error(`DSL 包含不支持的题型，无法创建：${unsupported.join("；")}。` +
            `骨架支持的题型：${supported}`);
    }
    const wire = [];
    let qIdx = 1;
 ...
 */
function isMatrixColumnHeader(line) {
        ...
        case "matrix-fill":
            ...
            if (body.length > 0 && (body[0] === "行：" || body[0].toLowerCase() === "rows:")) {
                ...
                for (const line of body) {
                    if (line === "行：" || line.toLowerCase() === "rows:")
                        continue;
                    if (line.startsWith("- ")) {
