...
function normalizeMatrixAnswerValue(rawValue) {
...
function normalizeRankingAnswerValue(rawValue) {
...
function extractBaseQuestionIndex(rawKey) {
...
function buildQuestionLookup(submitdata, questions) {
    const rawQuestions = new Map();
    const answerableQuestions = new Map();
    const rawToAnswerable = new Map();
    ...
    const mode = hasNonAnswerableQuestions && submittedBaseIndices.some((index) => index > answerableIndex && rawToAnswerable.has(index))
        ? "raw"
        : "answerable";
    return {
...
export function normalizeSubmitdata(submitdata, questions) {
...
//# sourceMappingURL=submitdata.js.map