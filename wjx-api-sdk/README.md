...
- **9 大模块、54+ 函数** — 覆盖问卷星 OpenAPI，并提供完整 WJX XML DSL 工作流
...
### WJX XML DSL — 默认 AI 创建与安全修改

| 函数 | 说明 |
|------|------|
| `queryWjxDsl(input)` | 使用传统 `vid` 查询问卷 DTO，并附带完整 DSL |
| `createSurveyByWjxDsl(input)` | 复用现有创建链路创建问卷，不生成幂等 ID |
| `updateWjxDsl(input)` | 使用传统 `vid` 和完整 DSL 复用现有修改链路；`If-Match` 为可选弱校验 Header |

普通 AI 新建调用 `createSurveyByWjxDsl`；修改已有问卷先用 `queryWjxDsl({ vid })` 获取完整 DSL，再调用 `updateWjxDsl`。写操作不会自动重试。
...
const dsl = await readFile("survey.wjx", "utf8");
const created = await createSurveyByWjxDsl({ dsl }, creds);
if (!created.result || created.data === undefined) {
  throw new Error("问卷创建失败或结果未知；不要自动重试");
}

const current = await queryWjxDsl({ vid: created.data.vid }, creds);
await updateWjxDsl({ vid: created.data.vid, dsl }, creds);
```

JSONL、旧文本 DSL 和旧 JSON 数组创建函数仍是兼容接口，仅在调用方明确选择相应格式时使用。
