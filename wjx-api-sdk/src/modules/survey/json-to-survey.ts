  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (normalized === "考试代码") {
      if (typeof obj.codetype !== "string" || !obj.codetype.trim()) {
        throw new Error(
          `JSONL 第 ${i + 1} 行考试代码必须提供非空字符串 codetype（编程语言）。` +
          `代码题没有标准答案设置；初始代码与判题配置须在 Web 编辑器补充。`,
        );
      }
      if (obj.correctselect !== undefined || obj.answer !== undefined || obj.answerlists !== undefined) {
        throw new Error(
          `JSONL 第 ${i + 1} 行考试代码不能设置 correctselect/answer/answerlists 标准答案；代码题应由运行/判题配置处理，不能降级为考试填空。`,
        );
      }
    ...
    if (normalized === "考试简答" && (obj.correctselect !== undefined || obj.answer !== undefined)) {
      throw new Error(
        `JSONL 第 ${i + 1} 行考试简答不能设置 correctselect/answer 标准答案；主观题请使用 answeranalysis/quizscore，人工阅卷配置请在 Web 编辑器完成。`,
      );
    }
    if (normalized === "简答题" && obj.isquiz === "1") {
      throw new Error(
        `JSONL 第 ${i + 1} 行普通简答题不能设置 isquiz="1"；考试主观题请改用 qtype="考试简答"，避免被服务端按考试填空路径处理。`,
      );
    }
    if (normalized === "考试单项填空" && obj.codetype !== undefined) {
      throw new Error(
        `JSONL 第 ${i + 1} 行考试单项填空不支持 codetype；需要编程语言的题目请使用 qtype="考试代码" 并提供 codetype。`,
      );
    }
