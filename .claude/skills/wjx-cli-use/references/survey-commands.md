...
用 JSONL 格式创建问卷（**唯一的 JSONL 创建方式**，登记 70+ 个 qtype；其中少数题型仅支持读取或 Web 编辑器配置）。每个非空行是一个完整 JSON 对象，首行必须是 `qtype` 为 `问卷基础信息` 的元数据。
...
普通题型在未指定 `--publish` 时默认立即发布。下列纯框架题型只有骨架，必须在问卷星编辑页补充素材或配置，未指定 `--publish` 时 SDK 会以草稿创建：`折叠栏目`、`轮播图`、`AI追问`、`AI处理`、`AI访谈`、`图片OCR`、`分页计时器`。`VlookUp问卷关联` 属于读取/Web 编辑器边界，当前 JSONL 创建接口明确拒绝，遇到它应转 Web 编辑器，不要重试。
