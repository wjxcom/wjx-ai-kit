...
/** 通用 JSONL 格式约束 */
const JSONL_CONSTRAINTS = "不要生成重复的题目以及无关的问题，同一题目不要生成相同的选项；relation/referselect 仅在用户提供并确认合法格式时原样保留，当前客户端不验证其语义；题目顺序需要遵循逻辑顺序，由简单到复杂，由客观到主观，开放题要放在末尾。";
...
题目数量硬性要求：必须生成 **${question_count ?? "15"} 道真实题目**（不计 _meta/分页/段落）。不要主动添加未经确认的 relation/referselect 逻辑字段；需要分支逻辑时请提示用户在 Web 编辑器中配置。推荐题型比例：单选（30%）、多选（25%）、量表/矩阵量表（15%）、矩阵单选（10%）、排序（5%）、填空（5%）、其它专业模型（10%，按主题取舍）。
...
3. 不要主动生成未经确认的 relation/referselect；关联逻辑请在 Web 编辑器中配置并验证
