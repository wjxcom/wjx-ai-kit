...
10. 【矩阵题 & 表格题 用法指南】qtype 必须使用下列精确名称之一，不要凭想象造新名字（如"矩阵""矩阵题""通用矩阵""表格题"等模糊值会创建失败）：
    ...
    ── 表格类（706-710 推荐使用统一字段描述格式） ──
    - 通用规则：
      1) qtype 使用精确名称：表格数值 / 表格填空 / 表格下拉框 / 表格组合 / 自增表格
      2) rowtitle = 字段名数组
      3) columntype = 与 rowtitle 一一对应的字段类型数组
      4) columndata = 与 rowtitle 一一对应的补充数据；无附加数据时传空字符串
      5) 下拉框/单选/多选的候选值优先放在 columndata，使用 | 分隔；如果整题共用一套下拉值，也可以直接用 select
      6) referselect 的值使用 "前题标题>>>字段名" 格式，例如 "队员信息表>>>姓名"
    - 表格下拉框：{"qtype":"表格下拉框","title":"队伍信息","rowtitle":["队伍名称","参赛组别","领队姓名","领队电话","紧急联系人电话"],"columntype":["单项填空","下拉框","姓名","手机","手机"],"select":["男子单打","女子单打","男子双打","女子双打","混合双打"]}
    - 自增表格：{"qtype":"自增表格","title":"队员信息表","rowtitle":["姓名","性别","年龄","联系电话","身份证号"],"columntype":["姓名","单选","表格数值","手机","身份证号"],"columndata":["","男|女","","",""],"requir":true}
    - 表格组合：{"qtype":"表格组合","title":"主力队员技术特点","rowtitle":["队员姓名","擅长技术","技术等级"],"columntype":["referselect","多选","表格下拉框"],"columndata":["队员信息表>>>姓名","杀球|吊球|搓球|推球|挑球|高远球","业余初级|业余中级|业余高级|专业级"],"requir":true}
    - 表格数值：{"qtype":"表格数值","title":"队员过往参赛经历","rowtitle":["比赛名称","参赛年份","所获名次"],"columntype":["表格填空","表格数值","表格下拉框"],"columndata":["","","冠军|亚军|季军|前八|未获奖"]}
    - 表格填空：{"qtype":"表格填空","title":"队伍紧急联系人信息","rowtitle":["联系人姓名","与队伍关系","联系电话"],"columntype":["姓名","表格下拉框","手机"],"columndata":["","教练|队长|队员家属|其他",""],"requir":true}
...
【重要】不要生成除 JSON 外的其他内容。生成完成后，请直接调用 create_survey_by_json 工具，将上述 JSONL 文本作为 jsonl 参数传入以创建问卷；考试/表单/测评/民主测评场景请同时显式传 atype 参数。`;

/** Survey 支持的 qtype 列表（覆盖 atype∈{1,2,10,11} 情形，含专业调查模型） */
...
/** Form 支持的 qtype 列表 */
const FORM_QTYPES = "单选、多选、下拉框、文件上传、单项填空、简答题、多项填空、矩阵填空、多级下拉、门店选择、签名题、地图、日期、AI追问、AI处理、分页栏、段落说明、折叠栏目、轮播图、图片OCR、商品题、预约题、姓名、基本信息、身份证号、国家及地区、省市、省市区、邮箱、手机、手机验证、时间、高校、邮寄地址、企业信息、知情同意书、密码、表格数值、表格填空、表格下拉框、表格组合、自增表格、多项文件题、多项简答题";
...
export function registerSurveyGenerationJsonPrompts(server: McpServer): void {
  // ═══ 1. Survey（调查/测评/量表/民主测评 — JSON 格式）═════════════════
  server.prompt(
    ...
    async ({ topic, question_count, requirements }) => ({
      messages: [{
        ...
        content: {
          type: "text",
          text: `你是一名专业的问卷调研专家，擅长制作用户调查、市场研究、学术调查、测评、量表、民主测评/360 评估等各类问卷。请根据主题「${topic}」生成一份高质量的问卷。${requirements ? `额外要求：${requirements}` : ""}
