...
questionnaire {
  ...
  advanced_types_note:
    "部分别名未覆盖的高级题型可用 Generic 别名 node \"Question\" 透传：基础 Type + 后端读取的标识属性（Mode/Verify/HasValue/IsCeShi/IsSignature/IsQingJing/IsEvaluate/IsLadder/IsTouPiao/IsShop/IsShelf/IsAppointment/Relation/Height）。是否可创建及页面是否可用必须以服务端响应和 Web 编辑器验收为准。",
