...
export function registerResources(server) {
    server.resource("jsonl-qtypes", "wjx://reference/jsonl-qtypes", { description: "JSONL 创建 qtype 能力与分层；区别于 get_survey 返回的 q_type/q_subtype 数字编码，并标明草稿/Web 编辑器限制", mimeType: "application/json" }, async () => ({ contents: [{ uri: "wjx://reference/jsonl-qtypes", mimeType: "application/json", text: formatResource(JSONL_QTYPES_RESOURCE) }] }));
    server.resource("survey-types", "wjx://reference/survey-types", { description: "问卷星支持的问卷类型列表（调查/测评/投票/考试/表单等）", mimeType: "application/json" }, async () => ({
        contents: [{
    ...
    server.resource("survey-statuses", "wjx://reference/survey-statuses", { description: "问卷状态编码说明：0=未发布, 1=已发布, 2=已暂停, 3=已删除（回收站，可恢复）, 4=彻底删除（不可恢复）, 5=被审核", mimeType: "application/json" }, async () => ({
        contents: [{
    ...
    server.resource("text-validation-types", "wjx://reference/text-validation-types", { description: "文本题校验类型编码（题目设置/读取结果）", mimeType: "application/json" }, async () => ({
        contents: [{
    ...
    server.resource("matrix-display-types", "wjx://reference/matrix-display-types", { description: "矩阵题展现形式编码", mimeType: "application/json" }, async () => ({
        contents: [{
    ...
    server.resource("table-display-types", "wjx://reference/table-display-types", { description: "表格题展现形式编码", mimeType: "application/json" }, async () => ({
        contents: [{
    ...
    server.resource("survey-setting-types", "wjx://reference/survey-setting-types", { description: "问卷设置内容类型编码（additional_setting）", mimeType: "application/json" }, async () => ({
        contents: [{
    ...
    server.resource("analysis-methods", "wjx://reference/analysis-methods", { description: "NPS/CSAT/CES 分析方法的计算公式与行业基准", mimeType: "application/json" }, async () => ({
        contents: [{
    ...
    server.resource("response-format", "wjx://reference/response-format", { description: "问卷星答卷 submitdata 字段的编码格式说明", mimeType: "application/json" }, async () => ({
        contents: [{
    ...
    server.resource("user-roles", "wjx://reference/user-roles", { description: "多账号子账号角色编码说明", mimeType: "application/json" }, async () => ({
        contents: [{
    ...
    server.resource("push-format", "wjx://reference/push-format", { description: "问卷星数据推送格式、AES加密与签名验证说明", mimeType: "application/json" }, async () => ({
        contents: [{
