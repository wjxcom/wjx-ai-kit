import { z } from "zod";
import { buildSsoSubaccountUrl, buildSsoUserSystemUrl, buildSsoPartnerUrl, buildSurveyUrl, buildPreviewUrl, getShortLink, } from "./client.js";
import { toolResult, toolError } from "../../helpers.js";
import { getRequestCredentials } from "../../core/context.js";
export function registerSsoTools(server) {
    // ─── sso_subaccount_url ───────────────────────────────────────────
    server.registerTool("sso_subaccount_url", {
        title: "生成子账号SSO登录链接",
        description: "生成子账号单点登录（SSO）链接，用于子账号免密登录或自动创建子账号。",
        inputSchema: {
    ...
    // ─── sso_user_system_url ──────────────────────────────────────────
    server.registerTool("sso_user_system_url", {
        title: "生成用户系统SSO链接",
        description: "生成用户系统参与者的单点登录（SSO）链接，用于用户系统中的成员免密登录。",
        inputSchema: {
    ...
    // ─── sso_partner_url ──────────────────────────────────────────────
    server.registerTool("sso_partner_url", {
        title: "生成合作伙伴SSO登录链接",
        description: "生成合作伙伴/代理商的单点登录（SSO）链接，用于合作伙伴免密登录。",
        inputSchema: {
    ...
    // ─── build_survey_url ─────────────────────────────────────────────
    server.registerTool("build_survey_url", {
        title: "生成问卷创建/编辑链接",
        description: "生成快速创建或编辑问卷的 URL（无需签名）。创建模式生成空白问卷链接，编辑模式生成问卷编辑器链接（需提供 activity 问卷编号）。",
        inputSchema: {
            mode: z
                .enum(["create", "edit"])
    ...
    // ─── build_preview_url ────────────────────────────────────────────
    server.registerTool("build_preview_url", {
        title: "生成问卷预览链接",
        description: "生成问卷预览链接（无需签名），可在浏览器中预览问卷效果。优先传 create_survey_* 返回的 sid；仅在没有 sid 时再传 vid。",
    ...
    // ─── get_short_link ───────────────────────────────────────────────
    server.registerTool("get_short_link", {
        title: "获取问卷短链接",
        description: "将问卷星问卷长链接转换为短链接，适合短信等场景发送。url 必须是问卷星问卷地址；链接中的查询参数会由接口自动编码。",
        inputSchema: {
            url: z
                .string()
                .trim()
