...
export function registerResponseTools(server) {
    ...
    server.registerTool("query_responses", {
        title: "答卷查询",
        description: "查询问卷的答卷数据，支持分页、时间范围、条件筛选。返回答卷明细包括提交时间、来源、IP、各题答案等。",
        inputSchema: {
        ...
        if (args.conds !== undefined)
            assertJson(args.conds, "conds");
        if (args.jid !== undefined && args.jid.split(",").length > 50)
            throw new Error("jid 最多允许传入50个");
        if (args.sojumpparm !== undefined && args.sojumpparm.split(",").length > 50)
            throw new Error("sojumpparm 最多允许传入50个");
        if (args.qid !== undefined && args.qid.split(",").length > 50)
            throw new Error("qid 最多允许传入50个");
    ...
    server.registerTool("query_responses_realtime", {
        title: "答卷实时查询",
        description: "实时查询新提交的答卷（队列模式）。查询成功的答卷将从队列移除，无法二次查询。需联系客服开通白名单。",
        inputSchema: {
            vid: z.number().int().positive().describe("问卷编号"),
    ...
    }, wrapToolHandler(async (args) => queryResponsesRealtime({
        vid: args.vid,
        count: args.count,
    })));
    // ─── download_responses ───────────────────────────────────────────
    server.registerTool("download_responses", {
        title: "答卷下载",
        description: "批量下载答卷数据，支持 CSV/SAV/Word 格式。超过3000条自动转为异步任务，返回 taskid 用于轮询。",
        inputSchema: {
            vid: z.number().int().positive().describe("问卷编号"),
    ...
    }, wrapToolHandler(async (args) => downloadResponses({
        vid: args.vid,
        taskid: args.taskid,
        valid: args.valid,
    ...
    server.registerTool("get_report", {
        title: "默认报告查询",
        description: "获取问卷的统计报告，包含各题选项频次统计、平均分、总分等聚合数据。注意：valid 参数建议显式传 true 以确保返回完整报告。",
        inputSchema: {
            vid: z.number().int().positive().describe("问卷编号"),
    ...
    // ─── submit_response ──────────────────────────────────────────────
    server.registerTool("submit_response", {
        title: "答卷提交",
        description: "向问卷提交答卷数据（代填/导入）。答卷格式：题号$答案}题号$答案，详见问卷星答卷格式规范。",
        inputSchema: {
            vid: z.number().int().positive().describe("问卷编号"),
    ...
    }, wrapToolHandler(async (args) => submitResponse({
        vid: args.vid,
    ...
    server.registerTool("get_winners", {
        title: "获取中奖者信息",
        description: "获取问卷的中奖者信息列表，支持按奖品类型和发放状态筛选，支持分页。",
        inputSchema: {
            vid: z.number().int().positive().describe("问卷编号"),
    ...
    }, wrapToolHandler(async (args) => getWinners({
        vid: args.vid,
        atype: args.atype,
    ...
    // ─── modify_response ──────────────────────────────────────────────
    server.registerTool("modify_response", {
        title: "修改答卷",
        description: "修改答卷数据。当前仅支持修改考试问卷的主观题分数（type=1）。answers 为 JSON 字符串，格式 {题号: 分数}。" +
    ...
    server.registerTool("get_360_report", {
        title: "360度评估报告下载",
        description: "下载360度评估报告的详细数据（XLS格式）。异步模式：首次调用可能返回 status=0 和 taskid，需再次调用并传入 taskid 轮询直到 status=1 获取下载链接。",
        inputSchema: {
            vid: z.number().int().positive().describe("问卷编号"),
            taskid: z.string().optional().describe("异步任务 ID（用于轮询下载状态）"),
        },
    ...
    }, wrapToolHandler(async (args) => get360Report({
        vid: args.vid,
        taskid: args.taskid,
    })));
    // ─── clear_responses ──────────────────────────────────────────────
    server.registerTool("clear_responses", {
        title: "清空答卷数据",
        description: "清空指定问卷的所有答卷数据。此操作不可逆！可选择是否重置答卷序号为0。",
        inputSchema: {
