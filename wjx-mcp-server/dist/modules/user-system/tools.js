import { z } from "zod";
import { addParticipants, modifyParticipants, deleteParticipants, bindActivity, querySurveyBinding, queryUserSurveys, } from "./client.js";
import { wrapToolHandler, assertJsonArray } from "../../helpers.js";
export function registerUserSystemTools(server) {
    // ─── add_participants ─────────────────────────────────────────────
    server.registerTool("add_participants", {
        title: "[已过时] 批量添加参与者",
        description: "[Deprecated] 向用户系统批量添加参与者（每次最多100人）。users 为 JSON 数组字符串，每个对象包含: uid(参与者唯一编号,必填)、uname(参与者姓名,选填)、upass(初始登录密码,选填)、udept(部门,选填,必须是数组如[\"部门1\"])、uextf(附加信息,选填,必须是数组如[\"信息1\"])。",
        inputSchema: {
            users: z
                .string()
                .min(2)
    ...
    // ─── modify_participants ──────────────────────────────────────────
    server.registerTool("modify_participants", {
        title: "[已过时] 批量修改参与者",
        description: "[Deprecated] 批量修改用户系统中参与者的信息。users 为 JSON 数组字符串，每个对象包含: uid(参与者唯一编号,必填)、uname(姓名)、upass(密码)、udept(部门,必须是数组格式如[\"部门1\"])、uextf(附加信息,必须是数组格式如[\"信息1\"])。auto_create_udept 可在部门不存在时自动创建。",
        inputSchema: {
            users: z
                .string()
                .min(2)
    ...
    // ─── delete_participants ──────────────────────────────────────────
    server.registerTool("delete_participants", {
        title: "[已过时] 批量删除参与者",
        description: "[Deprecated] 从用户系统中批量删除参与者。此操作不可逆，请谨慎使用！uids 为 JSON 数组字符串，如 [\"uid1\",\"uid2\"]。",
        inputSchema: {
            uids: z
                .string()
                .min(2)
    ...
    // ─── bind_activity ───────────────────────────────────────────────
    server.registerTool("bind_activity", {
        title: "[已过时] 绑定问卷到用户体系",
        description: "[Deprecated] 将问卷绑定到用户体系，并指定参与者。可设置作答次数限制、是否允许修改答案等。",
        inputSchema: {
    ...
    server.registerTool("query_survey_binding", {
        title: "[已过时] 查询问卷用户绑定",
        description: "[Deprecated] 查询问卷与用户系统的绑定关系，返回绑定的参与者列表。支持按参与状态(join_status)、日期(day)、周(week)、月(month)筛选，可选择是否强制获取参与次数(force_join_times)。",
        inputSchema: {
            vid: z.number().int().positive().describe("问卷编号"),
            usid: z.number().int().positive().describe("用户系统 ID（sysid）"),
    ...
    }, wrapToolHandler(async (args) => querySurveyBinding({
        vid: args.vid,
        sysid: args.usid,
    ...
    // ─── query_user_surveys ───────────────────────────────────────────
    server.registerTool("query_user_surveys", {
        title: "[已过时] 查询用户关联问卷",
        description: "[Deprecated] 查询指定参与者被分配的问卷列表，支持分页。",
        inputSchema: {
    ...
    }, wrapToolHandler(async (args) => queryUserSurveys({
        uid: args.uid,
        sysid: args.usid,
    })));
}
