...
wjx-mcp-server is a Model Context Protocol (MCP) server wrapping the Wenjuanxing (问卷星) OpenAPI. It exposes 57 tools, 8 resources, and 19 prompts to AI clients (Claude, Cursor, etc.). Pure TypeScript, ESM-only, minimal dependencies (`@modelcontextprotocol/sdk` + `zod`).
...
- `src/prompts/` — 19 prompts (4 general + 6 analysis + 2 operational + 7 survey generation)
