...
| **System** 系统配置 | 1 | 读取脱敏后的运行配置 |

### 9 个 MCP Resources

AI 客户端可直接读取的参考数据：`survey-types`、`question-types`、`survey-statuses`、`analysis-methods`、`response-format`、`user-roles`、`push-format`、`dsl-syntax`，以及版本化的 `wjx-xml-dsl`。其中 `dsl-syntax` 是旧文本 DSL 兼容参考。
...
flowchart LR
    ...
    Server --> R[Resources ×9]
    Server --> P[Prompts ×22]
    Server --> Tools[Tools ×61]
    ...
    S1 & D & S2 & S3 & S5 & S6 --> API[api-client.ts]
    API --> Sign[SHA1 签名]
    API --> WJX[(问卷星 OpenAPI)]
...
普通 AI 新建问卷默认使用 `wjx-dsl 1` 调用 `create_survey_by_wjx_dsl`；修改已有问卷使用 `query_wjx_dsl` 获取完整 DSL 后调用 `update_wjx_dsl`。`If-Match` 仅是可选弱前置校验，JSONL 和旧文本 DSL 工具只处理用户明确选择的兼容请求。
...
│   │   └── types.ts              # 共享类型定义
│   ├── modules/
│   │   ├── survey/               # 问卷管理（13 tools）
│   │   ├── dsl/                  # WJX XML DSL（3 tools）
│   │   ├── response/             # 答卷数据（9 tools）
│   │   ├── contacts/             # 通讯录（14 tools）
│   │   ├── sso/                  # 免登录 URL（5 tools）
│   │   ├── user-system/          # 用户体系（6 tools）
│   │   ├── multi-user/           # 多用户管理（5 tools）
│   │   └── analytics/            # 本地分析（5 tools；另含 1 个 get_config 系统工具）
│   ├── resources/                # 9 个 MCP Resources
