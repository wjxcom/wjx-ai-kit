...
- 统一 `wjx-api-sdk`、`wjx-mcp-server` 和 `wjx-cli` 的版本与工作区依赖至 `0.4.5`。
- 强化 XML DSL v1 的结构与题目语义校验，收紧 gapfill 占位符规范化，并在 SDK、CLI、MCP 创建链路中统一可创建问卷类型和更新字段。
- 同步 DSL 文档、Skill 镜像、发布产物与回归测试，减少导出、校验和创建流程之间的语义偏差。
