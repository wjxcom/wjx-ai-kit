...
普通题型未传 `--publish` 时默认立即发布。若 JSONL 包含纯框架题型（`折叠栏目`、`轮播图`、`AI追问`、`AI处理`、`AI访谈`、`图片OCR`、`分页计时器`），则默认创建为草稿，因为这些题型还需要在编辑页补充素材或配置。`VlookUp问卷关联` 当前由创建接口拒绝，只能读取既有问卷或在 Web 编辑器配置。完成二次编辑并获得用户明确授权后，再显式发布。
