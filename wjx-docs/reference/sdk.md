...
`wjx-cli` 当前源码版本为 `0.4.5`；发布后 CLI 用户可先执行 `npm install -g wjx-cli@latest`，成功后再执行 `wjx skill install --force` 获取 CLI 并启用 `wjx-cli-use`。npm 包名是 `wjx-cli`，安装后的命令名是 `wjx`，验证时运行 `wjx --version`。发布前按 [CLI 快速开始](../start/cli.md) 从源码构建。
