...
DSL 创建默认生成调查类型（用 `--type` 指定其它类型）；修改提交一个既有传统 `vid` 的完整 DSL，并调用服务端 action `A1000110`。该更新入口只适用于普通调查、投票、考试、表单等传统问卷；AI 主页必须使用 `wjx survey update-ai-page` 的 action `A1000108`，不能把 AI 主页 vid 传给 `wjx dsl update`。目标问卷是否属于服务端支持的 DSL 类型由后端最终判断；若返回题型不支持或 AI 主页类型错误，应停止重试并转 JSONL/Web 编辑器流程。DSL 与 JSONL（`survey create`）是两条相互独立的创建链路，互不转换。

更新前必须先读到匹配的传统问卷编号和完整 DSL；AI 主页或缺失 DSL 的读回不会进入写入动作。创建或更新后会再次只读查询服务端 DSL，并核对目标 `vid`、问卷标题、每道题显式给出的基础题型和属性，以及显式声明的行、列、选项及其顺序；服务端补充的默认属性不影响校验。缺少或改写这些内容时结果标记为 `unknown`，不得报告写入已验证。此读回不能证明预览、文件上传、滑动条或循环评价的作答页面已经可用；这些行为仍须单独验收。
