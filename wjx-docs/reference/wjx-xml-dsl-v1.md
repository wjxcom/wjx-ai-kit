...
支持 `//` 或 `#` 到行尾的注释。问卷可包含 `page { ... };` 分页块；分页块内的题目顺序就是该页的顺序，未声明分页时所有题目属于默认页。`questionnaire`、`page`、`question`、`item` 和 `raw` 块都必须以分号结束。属性值统一按字符串传输；布尔值使用 `"true"`/`"false"`，数字和问卷星编码也保持字符串形式，以避免不同语言客户端的类型转换差异。
...
逻辑动作字段（`if`、`show`、`hide`、`jump`、`branch`、`reference`、`random` 和 `raw`）可在查询结果中原样保留；当前 SDK/CLI/MCP 没有通用、可验证的逻辑修改接口，也没有客户端语法校验或写后读回。需要配置或修改分支、随机化和引用关系时，应使用 Web 编辑器并人工复核。
...
- **`RandomChoice`（选项随机）**：这是查询结果中的历史/服务端字段；当前 SDK/CLI/MCP 没有完整的创建、读取和验证闭环。读取既有问卷时可按原值解释：`1`（或 `true`）表示「全部选项随机」，`N>1` 表示「随机前若干项、保留末 `N−1` 项固定」，`0`/空表示不随机。不要在新建或修改流程中依赖该字段生效。
