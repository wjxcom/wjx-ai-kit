...
| 模块 | 工具（完整清单） | 说明 |
| --- | --- | --- |
| survey（13） | `create_survey_by_json`, `get_survey`, `list_surveys`, `update_survey_status`, `get_survey_settings`, `update_survey_settings`, `delete_survey`, `get_question_tags`, `get_tag_details`, `upload_file`, `clear_recycle_bin`, `create_ai_page`, `update_ai_page` | 问卷创建、查询、设置、生命周期和 AI 主页；新问卷统一使用 `create_survey_by_json` |
...
| sso（6） | `sso_subaccount_url`, `sso_user_system_url`, `sso_partner_url`, `build_survey_url`, `build_preview_url`, `get_short_link` | 生成登录、编辑、填写和短信短链接；用户系统 SSO 仅配合已有系统使用 |
...
| `wjx://reference/text-validation-types` | 文本题验证类型编码 |
| `wjx://reference/matrix-display-types` | 矩阵题展示形式编码 |
| `wjx://reference/table-display-types` | 表格题展示形式编码 |
| `wjx://reference/survey-setting-types` | 问卷设置类型编码 |
...
| `wjx://reference/jsonl-qtypes` | JSONL qtype 白名单、创建分层和 Web 编辑器边界 |
