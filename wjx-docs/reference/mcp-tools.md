...
| URI | 内容 |
| --- | --- |
| `wjx://reference/dsl-syntax` | 旧 `create_survey_by_text` 的 DSL 语法（已弃用） |
...
Prompt 是可复用的工作流模板，不能替代工具权限检查。问卷生成的新项目优先使用 JSON 模板；DSL 模板和用户体系模板仅作为兼容指导。
...
| DSL 生成（7，已弃用） | `generate-survey`, `generate-nps-survey`, `generate-360-evaluation`, `generate-satisfaction-survey`, `generate-engagement-survey`, `generate-exam-from-document`, `generate-exam-from-knowledge` |
| JSON 生成（3，推荐） | `generate-survey-json`, `generate-exam-json`, `generate-form-json` |
