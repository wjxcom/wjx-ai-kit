...
| `下拉框` | `select` | 下拉选择 |
| `排序` | `select` | 按顺序排列选项 |
| `单项填空` / `简答题` | `title` | 单行或多行文本；普通主观题使用 `简答题` |
...
| `矩阵数值题` | 706 | `rowtitle` | 只能读取既有问卷；JSONL 创建接口明确拒绝 |
...
以下题型可以通过 `create` 建立骨架，但仅凭 JSONL 示例不能完成素材、AI 参数、循环评价子题或代码判题配置：`折叠栏目`、`轮播图`、`AI追问`、`AI处理`、`AI访谈`、`图片OCR`、`分页计时器`、`循环评价`、`考试代码`。包含这些题型时，未显式指定发布选项会创建为草稿；创建后应先在编辑页完善，再由用户明确要求发布。`VlookUp问卷关联` 当前由 JSONL 创建接口拒绝，只能读取既有问卷或在 Web 编辑器配置。
...
`循环评价` 的 XML DSL 协议形态是 `matrix` + `Verify="circulate"`，可透传并读回协议结构，但评价对象、轮次和随机规则仍依赖服务端或网页配置；JSONL 题型名不能表达完整配置。创建或更新后应使用真实预览/答题页验收，不能仅凭 DSL 校验成功宣称运行时已支持。
...
- `correctselect`：考试选择题或单项填空的正确答案；`考试单项填空` 必填，不要用于 `考试简答` 或 `考试代码`；
- `quizscore`：题目分值；
- `answeranalysis`：答案解析；
- `codetype`：`考试代码` 必填的编程语言字符串；JSONL 服务端不保存 `code` / `initialcode`，请在 Web 编辑器设置初始代码；
- `include`、`answerlists`、`isaigrading`、`aiansweranalysis`：按题型需要使用。

考试主观题使用 `考试简答`，有标准答案的填空才使用 `考试单项填空` 或 `考试多项填空`。`考试简答` 与 `考试单项填空` 在 `get_survey` 中目前可能都返回 `q_type=5,q_subtype=5`，因此 CLI 只报告协议级读回结果，不会仅凭数字编码宣称语义已确认；代码题的判题/运行设置需要 Web 编辑器补充。
