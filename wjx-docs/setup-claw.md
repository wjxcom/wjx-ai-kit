...
打开你 Claw 工具的 AI 对话窗（OpenClaw / KimiClaw / QClaw / LinClaw / MaxClaw / EasyClaw 等），把下面这段话发给它：

````text
请帮我接入问卷星 MCP Server。我用的 Claw 工具是 <OpenClaw / KimiClaw / QClaw / LinClaw / MaxClaw / 其他>。
...
3. 找到我的 Claw 工具对应的 MCP 配置入口：
   - OpenClaw 特殊：编辑 ~/.openclaw/openclaw.json，配置嵌在 acp.mcpServers 下（注意是 acp.mcpServers，不是顶层 mcpServers）
   - 其他 Claw：标准 mcpServers 顶层
4. 把以下条目合并进去（OpenClaw 放 acp.mcpServers 下，其他放 mcpServers 下）：
   ```json
   "wjx": {
     "command": "npx",
...
> ⚠️ **OpenClaw 的坑**：MCP 配置必须放在 `acp.mcpServers`，复制其他 Claw 的配置直接放顶层 mcpServers 会读不到。让 AI 帮你判断最稳。
