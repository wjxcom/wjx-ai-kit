...
> **没有 Node.js？** CLI 需要 Node.js 20+ 环境。访问 [nodejs.org](https://nodejs.org) 下载最新 LTS 版本，安装后再执行上述命令。

安装完成后，根据你的 AI 工具，将 Skill 目录添加到对应位置：

**OpenClaw / KimiClaw / QClaw / LinClaw / MaxClaw**：
在工具的 Skills/技能 设置中，添加 `skills/wjx-cli-use` 目录路径。
...
2. 解压并提取其中的 `wjx-cli-use` 目录到 AI 工具的技能目录（如 `skills/wjx-cli-use/`）
3. 安装 CLI 并配置 API Key——复制以下内容发给你的 AI 助手：

> 请帮我安装问卷星 CLI：
> 1. 运行 `node --version` 检查 Node.js 版本，如果未安装或低于 20，请帮我安装 Node.js 20+
> 2. 运行 `npm install -g wjx-cli` 安装问卷星 CLI
> 3. 告诉我去这个链接获取 API Key：https://www.wjx.cn/weixinlogin.aspx?redirecturl=%2Fnewwjx%2Fmanage%2Fuserinfo.aspx%3FshowApiKey%3D1
> 4. 拿到 Key 后运行 `wjx init --api-key <Key>` 完成配置
