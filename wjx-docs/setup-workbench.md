...
## 🚀 最快接入：让工作台 AI 自己装（强烈推荐）

工作台用户大多不写代码——**最简单的方式就是让工作台 AI 自己装**。打开 Manus / WorkBuddy / QoderWork 的对话窗，把下面这段完整复制发给它：

````text
请帮我接入问卷星 MCP Server。我用的工作台是 <Manus / WorkBuddy / QoderWork>。

1. 帮我检查电脑上 Node.js 是否 ≥ 20，没装就引导我去 https://nodejs.org 装 LTS 版本
2. 引导我去这个链接微信扫码取 API Key 并发给你：
   https://www.wjx.cn/weixinlogin.aspx?redirecturl=%2Fnewwjx%2Fmanage%2Fuserinfo.aspx%3FshowApiKey%3D1
3. 拿到 Key 后，找到我这个工作台的 MCP / 工具配置入口（不同工作台位置不同，请你查一下），把以下条目加进去：
   ```json
   {
     "mcpServers": {
       "wjx": {
   ...
   ```
4. 配完提醒我**重启工作台**
5. 重启后让我对工作台说"创建一份客户满意度调查问卷"验证
````

> 工作台用户不需要看下面的手动步骤——除非 AI 说它不会改配置（极少情况）。
