...
## 🚀 最快接入：直接让 Claude Code 自己装

打开 Claude Code，把下面这段话发给它，全程不用读后面的文档：

````text
请帮我接入问卷星 MCP Server：
1. 检查 node --version >= 20，过低引导我去 https://nodejs.org 升级
2. 让我去 https://www.wjx.cn/weixinlogin.aspx?redirecturl=%2Fnewwjx%2Fmanage%2Fuserinfo.aspx%3FshowApiKey%3D1 微信扫码取 API Key（私有化部署用户把 www.wjx.cn 换成自己的域名）
3. 拿到 Key 后执行：
   claude mcp add wjx --env WJX_API_KEY=<我的Key> -- npx -y wjx-mcp-server@latest
   私有化域名用户加 --env WJX_BASE_URL=<我的域名>
4. 执行 `claude mcp list` 确认 wjx 已注册
5. 提醒我**重启当前 Claude Code 会话**（关闭重开），然后调用 list_surveys 验证
6. 同时执行 `npx wjx-cli skill install` 安装专家 Agent + Skill 包
````

> **为什么要重启**：Claude Code 启动时一次性拉起 MCP server 子进程，新加的 wjx 要新会话才能加载。配置里写了 `wjx-mcp-server@latest`，每次启动会自动取最新版。
...
2. **Node.js >= 20** — 运行 `node --version` 确认版本。如果未安装或版本低于 20，推荐以下方式安装：
   - **使用 nvm（推荐）**：`nvm install 20 && nvm use 20`（[nvm 安装说明](https://github.com/nvm-sh/nvm)，Windows 用户可使用 [nvm-windows](https://github.com/coreybutler/nvm-windows)）
   - **官网下载**：前往 [nodejs.org](https://nodejs.org) 下载 LTS 版本安装包
...
# 企业用户（需要通讯录功能）
claude mcp add wjx --env WJX_API_KEY=你的APIKey --env WJX_CORP_ID=你的企业ID -- npx -y wjx-mcp-server@latest
```
...
从 [GitHub 仓库](https://github.com/wjxcom/wjx-ai-kit) 下载以下文件到你的项目目录：
...
```bash
claude mcp add wjx --env WJX_API_KEY=自己的APIKey -- npx -y wjx-mcp-server@latest
```
...
2. 确认文件名正确：`wjx-cli-expert.md`（`skill install` 安装），如需 MCP 专家则手动复制 `wjx-mcp-expert.md`
