...
MCP 是 Anthropic 提出的 AI 工具调用协议。wjx-mcp-server 实现了 **{{MCP_TOOL_COUNT}} 个工具**，让 Claude、Cursor、Windsurf 等 AI 客户端可以直接操作问卷星。

- **{{MCP_TOOL_COUNT}} 个 Tools** + **{{MCP_RESOURCE_COUNT}} 个 Resources** + **{{MCP_PROMPT_COUNT}} 个 Prompts**
...
wjx-cli 是为 AI Agent 设计的命令行工具。{{CLI_COMMAND_COUNT}} 个子命令覆盖问卷星全部 API 能力，输出结构化 JSON，天然适配 AI Agent 工作流和自动化脚本。

- **{{CLI_COMMAND_COUNT}} 个子命令**：问卷、答卷、通讯录、部门、管理员、标签、用户体系、子账号、SSO、数据分析
- **AI Agent 友好**：JSON 输出 + 管道输入 + 结构化错误 + Shell 补全
- **{{CLI_ANALYTICS_COUNT}} 个离线分析命令**：NPS、CSAT、异常检测、答卷解码等不需要 API Key

**适合谁：** AI Agent 开发者、自动化脚本编写者、命令行爱好者

### SDK — 零依赖 TypeScript 基础层

wjx-api-sdk 是零运行时依赖的 TypeScript SDK，提供 {{SDK_FUNCTION_COUNT}} 类型安全的函数，覆盖问卷星 OpenAPI 全部能力。
...
| 层级 | 能力 | 说明 |
|------|------|------|
| **MCP 工具层** | {{MCP_TOOL_COUNT}} 个 Tools + {{MCP_RESOURCE_COUNT}} Resources + {{MCP_PROMPT_COUNT}} Prompts | 所有 MCP 客户端通用，AI 直接调用 |
...
| 异常检测 | ✅ | ✅ | ✅ |
| Webhook 解密 | ✅ | ✅ | ✅ |
| AI Prompt 模板 | - | ✅ {{MCP_PROMPT_COUNT}} 个 | - |
| AI 参考资源 | - | ✅ {{MCP_RESOURCE_COUNT}} 个 | - |
| Claude Code 技能 | - | - | ✅ |
| Shell 补全 | - | - | ✅ |
| Docker 部署 | - | ✅ | - |
...
- [MCP Server 进阶指南](./mcp-advanced.md) — {{MCP_TOOL_COUNT}} 个工具深度用法、HTTP 部署、Docker
- [CLI 进阶指南](./cli-advanced.md) — {{CLI_COMMAND_COUNT}} 个命令完全攻略、Skill 系统
