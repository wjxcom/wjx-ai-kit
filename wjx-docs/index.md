...
当前版本提供 MCP **{{MCP_TOOL_COUNT}} 个 Tool**、**{{MCP_RESOURCE_COUNT}} 个 Resource**、**{{MCP_PROMPT_COUNT}} 个 Prompt**，以及 CLI **{{CLI_COMMAND_COUNT}} 个叶子命令**；SDK 的公开导出以 [SDK API 参考](reference/sdk.md) 为准。CLI 是主入口，MCP 只保证核心业务子集；完整差异见仓库 `capabilities/capability-matrix.json`。
