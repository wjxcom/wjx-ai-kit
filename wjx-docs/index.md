...
当前版本提供 MCP **66 个 Tool**、**13 个 Resource**、**15 个 Prompt**，以及 CLI **81 个叶子命令**；SDK 的公开导出以 [SDK API 参考](reference/sdk.md) 为准。CLI 是主入口，MCP 只保证核心业务子集；完整差异见仓库 `capabilities/capability-matrix.json`。

问卷创建支持 JSONL 和完整 XML DSL 两条独立路径；XML DSL 规范见 [WJX XML DSL v1](reference/wjx-xml-dsl-v1.md)。
