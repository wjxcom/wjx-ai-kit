...
### get_short_link — 获取问卷短链接

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `url` | string (URL) | 是 | 问卷星问卷填写长链接，可包含查询参数 |

调用 `openapi/shortlink.aspx` 将问卷长链接转换为短链接，返回接口的 `success`、`msg` 和 `data` 字段。输入必须是问卷星问卷地址；不要将后台编辑链接或其他站点地址提交给该工具。
...
检测规则：直线作答（所有答案相同）、速度异常（仅在至少 3 个正数耗时样本且中位数大于 0 时，<中位数 30%；样本不足返回 warning 并跳过速度标记）、IP+内容重复。
...
输出差值、变化率和启发式阈值标记（|变化率|>10% 为显著，不代表统计显著性检验）。
