...
MCP 服务会在 OpenAPI 1001001 请求中自动携带 `submit_channel=wjx-mcp`，用于标记答卷提交来源。

### submitdata 编码格式

**题号**：必须用 `get_survey` 返回的原始 `q_index`。不同问卷的题号可能从 1、2 或其他服务端分配值开始，不能假设元数据占 1。**不要**按"第几题"自己数 1, 2, 3，会与服务端 q_index 错位被拒。
...
**单题示例**（`Q` 是 `get_survey` 返回的 `q_index`；下面的 2 只是示例值）：
...
当前仅支持修改考试问卷的主观题分数（type=1）。这是高风险写入：服务会先按 `vid + jid` 读取并确认目标，再执行一次修改，最后读回并逐项核对 `answers`；只有返回 `outcome: "verified"` 才能向用户报告成功。
...
目标不存在时，服务只执行读操作并停止，不会发出修改请求。读回缺少答案字段或值不一致时返回 `outcome: "unknown"`，应停止重试并人工复核。
