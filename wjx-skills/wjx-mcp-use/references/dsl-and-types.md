# DSL 语法与题型参考

## DSL 文本语法（仅历史读取/迁移兼容）

> 本页只用于读取、审阅和迁移历史 DSL。新问卷请使用 JSONL；当前没有 DSL 创建入口。用户体系（`atype=8`）同样不能通过创建接口新建。

### 结构规则

- 第一行：问卷标题
- 空行后：问卷描述（可选）
- 空行后：题目列表
- 题目之间用空行分隔
- 分页用 `=== 分页 ===` 标记
- 不带序号的段落为引导语/段落说明（**注意**：`[段落说明]` 在创建时会被自动过滤，API 不支持段落类型）
- `（选填）` 标记非必填题

### 题目格式

```
序号. 题目标题[题型标记]
选项A
选项B
```

量表题选项用 `min~max` 格式（如 `1~5`）。

### 题型标记（28 种）

| 标记 | API 编码 |
|------|----------|
| `[单选题]` | q_type=3, q_subtype=3（可省略，默认题型） |
| `[多选题]` | q_type=4, q_subtype=4 |
| `[下拉框]` | q_type=3, q_subtype=301 |
| `[量表题]` | q_type=3, q_subtype=302 |
| `[评分单选]` | q_type=3, q_subtype=303 |
| `[情景题]` | q_type=3, q_subtype=304 |
| `[判断题]` | q_type=3, q_subtype=305 |
| `[评分多选]` | q_type=4, q_subtype=401 |
| `[排序题]` | q_type=4, q_subtype=402 |
| `[商品题]` | q_type=4, q_subtype=403 |
| `[填空题]` | q_type=5, q_subtype=5 |
| `[简答题]` | q_type=5 的别名 |
| `[问答题]` | q_type=5 的别名 |
| `[多级下拉题]` | q_type=5, q_subtype=501 |
| `[多项填空题]` | q_type=6, q_subtype=6（标题须含 `{_}` 占位符） |
| `[考试多项填空]` | q_type=6, q_subtype=601 |
| `[考试完形填空]` / `[完形填空]` | q_type=6, q_subtype=602 |
| `[矩阵题]` | q_type=7, q_subtype=7 |
| `[矩阵单选题]` | q_type=7, q_subtype=702 |
| `[矩阵多选题]` | q_type=7, q_subtype=703 |
| `[矩阵量表题]` | q_type=7, q_subtype=701 |
| `[矩阵填空题]` | q_type=7, q_subtype=704 |
| `[文件上传]` | q_type=8, q_subtype=8 |
| `[绘图题]` | q_type=8, q_subtype=801 |
| `[比重题]` | q_type=9, q_subtype=9 |
| `[滑动条]` | q_type=10, q_subtype=10 |

括号兼容：支持 `[]`、`【】`、`()`、`（）` 四种括号。

### DSL 示例

```
员工满意度调查

请根据您的实际体验填写以下问卷。

1. 您的部门[下拉框]
技术部
市场部
销售部
人事部

2. 您对工作环境的满意度[量表题]
1~5

3. 您认为最重要的福利是？[多选题]
薪资
假期
培训机会
健康保险

=== 分页 ===

4. 请对以下维度评分[矩阵量表题]
行：
- 沟通效率
- 团队协作
- 管理支持

5. 您的改进建议[填空题]（选填）
```

### DSL 限制

DSL 是问卷的「可读摘要」，以下高级设置不在 DSL 中表示：
- 分支逻辑（跳转规则）
- 验证规则（正则、长度限制）
- 评分规则和权重
- 随机化设置

以上需通过 `get_survey` 的 JSON 格式获取和修改。

### 投票问卷

投票问卷使用 `atype: 3`，题目仍然使用普通题型标签：

```
最佳员工投票

请投出您的一票。

1. 最佳个人奖[单选题]
张三
李四
王五

2. 最佳团队（可多选）[多选题]
技术团队
设计团队
运营团队
```

当前 MCP Server 不提供 DSL 创建工具。历史 DSL 需要在 MCP 外部转换为 JSONL，再调用 `create_survey_by_json`；不要把 DSL 文本直接传给 MCP。

**注意**：投票问卷的题目就是普通 `[单选题]`/`[多选题]`，不存在 `[投票单选题]` 标签。`atype: 3` 控制问卷展示为投票样式，题型本身不变。

### 不支持的题型

以下题型**无法通过历史 DSL 表达**，遇到时应告知用户并建议在 JSONL 或网页端补充；这不是对当前 JSONL API 题型能力的否定：

| 题型 | 状态 | 替代方案 |
|------|------|---------|
| 签名题 | 不支持 | 用 `[绘图题]` 近似替代 |
| 地区题/省市区选择 | 不支持 | 用 `[多级下拉题]` 或网页端手动添加 |
| NPS 专用题 | 不支持 | 用 `[量表题]` + `0~10` 实现相同效果 |
| 自动每题一页 | 不支持 | 用 `=== 分页 ===` 在每题间手动分页 |
| 段落说明 | DSL 解析后被过滤 | API 不支持段落类型，内容不会出现在问卷中 |

---

## 问卷类型编码

| 编码 | 类型 |
|------|------|
| 1 | 调查 |
| 2 | 测评 |
| 3 | 投票 |
| 4 | 360度评估 |
| 5 | 360评估(无测评关系) |
| 6 | 考试 |
| 7 | 表单 |
| 8 | 用户体系 |
| 9 | 教学评估 |
| 10 | 量表 |
| 11 | 民主评议 |

## 问卷状态码

| 编码 | 状态 |
|------|------|
| 0 | 未发布 |
| 1 | 已发布 |
| 2 | 已暂停 |
| 3 | 已删除（回收站，可恢复） |
| 4 | 彻底删除（不可恢复） |
| 5 | 被审核 |

## 审核状态

| 编码 | 状态 |
|------|------|
| 1 | 已通过 |
| 2 | 审核中 |
| 3 | 未通过 |
| 4 | 待实名 |

## 题型完整映射表（q_type / q_subtype）

| q_type | 名称 | q_subtype |
|--------|------|-----------|
| 1 | 分页 | — |
| 2 | 段落 | — |
| 3 | 单选题 | 3=单选题, 301=下拉框, 302=量表题, 303=评分单选, 304=情景题, 305=判断题 |
| 4 | 多选题 | 4=多选题, 401=评分多选, 402=排序题, 403=商品题 |
| 5 | 填空题 | 5=填空题, 501=多级下拉题 |
| 6 | 多项填空题 | 6=普通多项填空, 601=考试多项填空, 602=考试完形填空 |
| 7 | 矩阵题 | 701=矩阵量表, 702=矩阵单选, 703=矩阵多选, 704=矩阵填空, 705=矩阵滑动条, 706=矩阵数值, 707=表格填空, 708=表格下拉框, 709=表格组合, 710=表格自增, 711=多项文件, 712=多项简答 |
| 8 | 文件上传 | 8=文件上传, 801=绘图题 |
| 9 | 比重题 | — |
| 10 | 滑动条 | — |

## 文本校验类型

用于文本题的校验/输入类型设置（不是 JSONL `qtype`）：

| 编码 | 类型 | 编码 | 类型 |
|------|------|------|------|
| 0 | 不验证/单行文本 | 1 | 数字 |
| 2 | 小数 | 3 | 日期 |
| 4 | 手机 | 5 | 指定选项 |
| 6 | 固话 | 7 | 电话 |
| 8 | Email | 9 | 密码 |
| 10 | 城市单选 | 11 | 省市区 |
| 12 | 高校 | 13 | 地图 |
| 14 | 网址 | 15 | 身份证号 |
| 16 | 学号 | 17 | QQ |
| 18 | 汉字 | 19 | 姓名 |
| 20 | 英文 | 21 | 英文数字 |
| 22 | 文件上传 | 23 | 多项简答 |
| 24 | 多级下拉 | 25 | 单行文本 |
| 26 | 多行文本 | | |

## 矩阵展现形式

| 编码 | 展现形式 |
|------|----------|
| 0 | 无 |
| 101 | 矩阵量表 |
| 102 | 矩阵多选 |
| 103 | 矩阵单选 |
| 201 | 矩阵填空 |
| 202 | 矩阵滑动条 |
| 203 | 多项文件 |
| 204 | 多项简答 |
| 301 | 表格数值 |
| 302 | 表格填空 |
| 303 | 表格下拉框 |

## 表格展现形式

| 编码 | 展现形式 |
|------|----------|
| 0 | 无 |
| 1 | 表格组合 |
| 2 | 表格自增 |

## 问卷设置参数类型

传给 `get_survey_settings` 的 `additional_setting` 是 JSON 数组；默认 `[1000,1001,1002,1003,1004,1005,1006,1007]`。

| 编码 | 设置内容 |
|------|----------|
| 1000 | 问卷时间设置 |
| 1001 | 问卷提交后处理 |
| 1002 | 成绩单设置 |
| 1003 | 维度设置 |
| 1004 | 自定义链接参数问卷答题设置 |
| 1005 | 奖品设置 |
| 1006 | 数据推送设置 |
| 1007 | 问卷所在文件夹 |
