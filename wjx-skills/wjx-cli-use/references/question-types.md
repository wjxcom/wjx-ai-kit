...
`wjx survey create-by-json --file <jsonl>`（显式 JSONL 兼容路径，覆盖 70+ 题型）和老命令 `wjx survey create --questions <json>`（向后兼容）中使用的 q_type 和 q_subtype 代码完整映射表。

> JSONL 路径每行一道题，字段名同下表；JSON 路径传题目数组。普通 AI 新建默认使用 WJX XML DSL，本页仅供显式兼容格式查码。
