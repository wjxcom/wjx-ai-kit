...
**多项填空关键规则**：
- 子填空位数量 = q_title（JSON 方式）或 title（JSONL 方式）中 `{_}` 占位符的数量。例：`"电话 {_}，邮箱 {_}，微信 {_}"` 会生成 3 个输入框。
- **禁止使用 `rowtitle` 数组定义子项** — `rowtitle` 是矩阵题/比重题/Kano/PSM 的字段，多项填空不支持；写了会被服务端忽略并只生成 1 个空位。
- JSONL 正确写法：`{"qtype":"多项填空","title":"电话 {_}，邮箱 {_}，微信 {_}"}`
- JSONL 错误写法（不要这样写）：`{"qtype":"多项填空","title":"请完善联系方式","rowtitle":["电话","邮箱","微信号"]}`
