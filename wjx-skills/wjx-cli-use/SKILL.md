...
无论用户要求多少种题型，**必须在一次 `create-by-json`（首选）或 `create-by-text` 调用中包含所有题目**。一个问卷可包含任意数量、任意类型的题目。
...
| 用户意图 | 命令 |
|---------|------|
| 做调查/问卷 | `wjx survey create-by-json --questions '<JSON>'`（首选，支持 70+ 题型）；简单场景可用 `create-by-text` |
| 做考试/测验 | `wjx survey create-by-json --questions '<JSON>' --type 6`（首选）；简单场景可用 `create-by-text --type 6` |
| 做投票 | `wjx survey create-by-json --questions '<JSON>' --type 3`（首选）；简单场景可用 `create-by-text --type 3` |
| 做表单/报名表 | `wjx survey create-by-json --questions '<JSON>' --type 7`（首选）；简单场景可用 `create-by-text --type 7` |
...
| 模块 | 命令 | 说明 |
|------|------|------|
| `survey` | list, get, create, create-by-json（推荐）, create-by-text, delete, status, settings, update-settings, upload, export-text, url | 问卷增删改查与配置 |
...
> **重要**：必须执行 `wjx survey create-by-json` 或 `create-by-text` 命令来创建问卷。只生成题目数据而不执行命令，问卷不会被创建到问卷星平台上。

**首选方式：JSON 创建**（支持 70+ 题型，覆盖全部 q_type/q_subtype 编码）

```bash
wjx survey create-by-json --title "问卷标题" --questions '[{"q_index":1,"q_type":3,"q_subtype":3,"q_title":"题目文本","is_requir":true,"items":[{"q_index":1,"item_index":1,"item_title":"选项A"},{"q_index":1,"item_index":2,"item_title":"选项B"}]}]'
```

问卷类型：`--type 1` 调查（默认），`3` 投票，`6` 考试，`7` 表单。

完整 q_type/q_subtype 编码见 [references/question-types.md](references/question-types.md)。
