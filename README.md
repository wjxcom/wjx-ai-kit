...
├── wjx-mcp-server/    # MCP Server（64 Tools / 9 Resources / 22 Prompts）
...
AI 新建问卷默认使用版本化的 **WJX XML DSL**；公开 DSL 接口只有 query、create、update。

```bash
wjx dsl query --vid 123456
wjx dsl create --file survey.wjx
wjx dsl update --vid 123456 --file survey.wjx
```

JSONL（`survey create-by-json`）、旧文本 DSL（`survey create-by-text`）和旧 JSON 数组接口继续保留，但只在用户明确选择对应格式时使用。创建或保存结果未知时不会自动切换格式或重试写操作，避免生成重复问卷。
...
| [MCP 进阶](wjx-docs/mcp-advanced.md) | （可选）64 工具深度用法、HTTP 部署、Docker |
