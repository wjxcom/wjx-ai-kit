...
test("generated JSONL profile is source aligned and distinguishes qtype tiers", () => {
  ...
  for (const key of ["qtypes", "creatableQtypes", "creatableAtypes", "tiers", "frameworkQtypes", "npsRules", "sourceRevision"]) assert.ok(key in profile, `missing ${key}`);
  ...
  assert.ok(profile.creatableQtypes.includes("NPS量表"));
  assert.ok(!profile.creatableQtypes.includes("VlookUp问卷关联"));
  assert.ok(!profile.tiers["framework-draft"].qtypes.includes("VlookUp问卷关联"));
  assert.ok(profile.tiers["read-only-or-web-editor"].qtypes.includes("VlookUp问卷关联"));
