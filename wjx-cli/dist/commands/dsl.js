...
function stdinData(command) {
    return command.__stdinData;
}
function hasGlobalFlag(command, name) {
    let current = command;
    while (current) {
        if (current.opts()[name] === true)
            return true;
        current = current.parent;
    }
    return false;
}
function normalizeDsl(value, source) {
    if (typeof value !== "string")
        throw new CliError("INPUT_ERROR", `${source} 必须是字符串`);
    const dsl = value.replace(/^\uFEFF/, "");
    if (!dsl.trim())
        throw new CliError("INPUT_ERROR", `${source} 中的 DSL 不能为空`);
    if (Buffer.byteLength(dsl, "utf8") > MAX_DSL_BYTES)
        throw new CliError("INPUT_ERROR", `${source} 中的 DSL 超过 4 MiB 限制`);
    return dsl;
}
function readDslFile(pathValue) {
    if (typeof pathValue !== "string" || !pathValue.trim())
        throw new CliError("INPUT_ERROR", "--file 必须是非空路径");
    try {
        if (statSync(pathValue).size > MAX_DSL_BYTES + 4)
            throw new CliError("INPUT_ERROR", "DSL 文件超过 4 MiB 限制");
...
function resolveDsl(command, actionOptions) {
    ...
    if (stdinRequested && (hasDsl || hasFile))
        throw new CliError("INPUT_ERROR", "--stdin 与 --dsl/--file 互斥");
    if (Number(hasDsl) + Number(hasFile) + Number(stdinDsl) !== 1)
        throw new CliError("INPUT_ERROR", "必须且只能通过 --dsl、--file 或 stdin JSON 提供 DSL");
    if (hasFile)
        return readDslFile(actionOpts.file);
    return normalizeDsl(hasDsl ? actionOpts.dsl : fromStdin?.dsl, hasDsl ? "--dsl" : "stdin JSON");
}
function requireTraditionalVid(value) {
    if (typeof value !== "string" || !value.trim() || value.trim() !== value || /[\u0000-\u001f\u007f]/.test(value)) {
        throw new CliError("INPUT_ERROR", "--vid 必须是有效的传统问卷 vid");
    }
    return value;
}
function addDslInput(command) {
    return command.option("--dsl <text>", "AI 生成的 WJX XML DSL").option("--file <path>", "从 UTF-8 文件读取 WJX XML DSL");
}
export function registerDslCommands(program) {
    ...
    addDslInput(dsl.command("generate").description("校验并规范化 AI 生成的 DSL").option("--out <path>", "将规范化 DSL 写入文件")).action(async (_options, command) => {
        try {
            ...
            if (!result.valid)
                throw new CliError("INPUT_ERROR", result.diagnostics.map((item) => item.message).join("；"), { data: result });
            if (typeof command.opts().out === "string")
                writeFileSync(command.opts().out, result.dsl, "utf8");
            if (options.format === "table" || command.opts().out === undefined)
    ...
    addDslInput(dsl.command("create").description("提交 AI 生成的 WJX XML DSL 创建问卷").option("--type <n>", "问卷类型", strictInt).option("--publish", "创建后发布").option("--compress-img", "压缩图片")).action(async (_options, command) => {
        ...
        await executeRuntimeAction(program, command, createSurveyByWjxDsl, () => ({
            ...
            ...(merged.publish === undefined ? {} : { publish: merged.publish }),
            ...((merged.compress_img ?? merged.compressImg) === undefined ? {} : { compress_img: (merged.compress_img ?? merged.compressImg) }),
        }));
    });
