...
# 问卷类型 (atype) — 通过 --type 参数设置
  ...
  atype=1   调查问卷
  atype=2   测评
  atype=3   投票          ← JSONL 用 qtype=投票单选/投票多选；DSL 用普通[单选题]/[多选题]
  atype=4   360度评估
  atype=5   360评估(无测评关系)
  atype=6   考试          ← 题目仍用普通[单选题]/[填空题]等
  atype=7   表单
  atype=8   用户体系（历史兼容；创建接口不支持新建）
  atype=9   教学评估
  atype=11  民主评议
...
考试支持评分单选(303)、评分多选(401)、判断题(305)等；JSONL 考试题可用 \`correctselect\` 设置正确答案、用 \`quizscore\` 设置题目分值。`,
    },
    "survey": {
        title: "survey 模块命令参考",
...
## wjx survey list
  ...
  --name_like <s>  名称模糊搜索
  --time_type <n>  时间类型：0=不按时间查询（默认）, 1=按问卷开始时间, 2=按问卷创建时间
...
## wjx survey create
  用 JSON 创建问卷（向后兼容，新项目请用 create-by-json 或 create-by-text）。
  --title <s>      问卷标题（必填）
  --type <n>       问卷类型（默认 0）
  --description <s> 问卷描述
...
## wjx survey create-by-json (唯一推荐)
  用 JSONL 格式创建问卷，每行一题，覆盖 70+ 题型（含矩阵/比重/滑块/文件上传/排序等全部场景）。
  --jsonl <s>      JSONL 字符串内容
...
## wjx survey create-by-text (已弃用)
  用 DSL 文本创建问卷。新项目请改用 create-by-json，本命令仅为兼容保留。
...
## wjx response submit
  ...
  示例：
    ...
    排序："4$2|3|1" = 第4题排序：选项B第1名、选项C第2名、选项A第3名（竖线分隔，按排名顺序列出所有选项序号）
...
## wjx analytics anomalies
  ...
  检测模式：直线作答、速度异常（耗时低于中位数 30%）、IP+内容重复
  答卷字段：answers 或 submitdata；duration_seconds 或 inputcosttime；可选 id/jid、ip
