...
function parseScoreArray(value: unknown, field: string, min: number, max: number): number[] {
...
function parseScale(value: unknown): "5-point" | "7-point" {
  if (value === undefined) return "5-point";
  ...
  analytics
    ...
    .action(async (_opts, cmd) => {
      await executeRuntimeLocal(program, cmd, (m) => {
        ...
        return compareMetrics(setA as Record<string, number>, setB as Record<string, number>);
      });
    });
