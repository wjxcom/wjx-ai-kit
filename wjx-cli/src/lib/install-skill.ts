...
function copySkillForInstall(source: string, destination: string, existing: string): string[] {
...
export function installSkill(
  ...
  if (force || !agentExists) {
    targets.push({ destination: agentDest, stage: (path: string) => {
  ...
  if (force || !skillExists) {
    targets.push({
      ...
      stage: (path: string) => copySkillForInstall(skillSrc, path, skillDest),
    });
  }
  if (force || !claudeSkillExists) {
    targets.push({ destination: claudeSkillDest, stage: (path: string) => copyDirectory(skillSrc, path) });
  }
