...
大多数命令需要问卷星 API Token（SSO 和 analytics 命令除外）。

```bash
# 方式 1：环境变量（推荐）
export WJX_TOKEN=your_api_token

# 方式 2：命令行参数
wjx --token your_api_token survey list

# 方式 3：.env 文件（SDK 自动读取）
echo "WJX_TOKEN=your_api_token" >> .env
```
...
| 选项 | 说明 |
|------|------|
| `--token <token>` | WJX API Token（或设置 `WJX_TOKEN` 环境变量） |
| `--json` | JSON 输出（默认） |
...
> **无需 Token** — SSO 命令在本地生成 URL，不调用 API。
...
> **无需 Token** — analytics 命令在本地执行计算，不调用 API。
...
```bash
wjx whoami    # 验证 Token 并显示账号信息（调用 listSurveys 检测）
wjx doctor    # 环境诊断（Node 版本、Token、API 连接、SDK 版本）
```
...
| **1** | `AUTH_ERROR` | 认证错误 | Token 缺失或无效 |
| **2** | `INPUT_ERROR` | 输入错误 | 缺少必填参数、无效整数、JSON 解析失败 |

### 示例

```bash
# 缺少 Token → exit 1, AUTH_ERROR
$ wjx survey list 2>&1 >/dev/null
{"error":true,"message":"WJX_TOKEN 未设置。请通过 --token 参数或 WJX_TOKEN 环境变量提供。","code":"AUTH_ERROR","exitCode":1}
...
WJX_TOKEN=xxx node dist/index.js survey list
```
