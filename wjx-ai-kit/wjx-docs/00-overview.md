...
MCP (Model Context Protocol) 是 Anthropic 提出的 AI 工具调用协议。wjx-mcp-server 实现了 **57 个工具**，让 Claude、Cursor、Windsurf 等 AI 客户端可以直接操作问卷星。

- **57 个 Tools**：创建问卷、查询答卷、管理通讯录、生成 SSO 链接、本地数据分析
...
| 层级 | 能力 | 说明 |
|------|------|------|
| **MCP 工具层** | 57 个 Tools + 8 Resources + 19 Prompts | 所有 MCP 客户端通用，AI 直接调用 |
...
AI 会自动执行安装，然后等你提供 API Key。微信扫码登录后，把 Key 发给 AI（例如"这是 API Key：sk-wjx-xxx，请继续"），AI 就能帮你完成配置。如果你使用自定义域名（非 www.wjx.cn），请在提示中告诉 AI。
...
57 个工具 + 8 个参考资源 + 19 个预设 Prompt，AI 直接调用。
...
- [MCP Server 进阶指南](./mcp-advanced.md) — 57 个工具深度用法、HTTP 部署、Docker
