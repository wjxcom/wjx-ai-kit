...
设计思路：如果用户没有给出调研目的，请根据主题推测出调研目的，然后结合主题和调研目的推测出场景和对象。然后根据目的、场景和对象，对主题进行分类。根据分类后的主题，从基本信息开始，逐步到意向、决策因素，然后是服务评估，最后是主观反馈。遵循逻辑顺序：按照主题分类从简单到复杂，从基本到敏感。
...
设计思路：如果用户没有给出维度指标，请根据主题生成维度指标。然后根据主题和维度指标，推测出与主题相关的调研目的、场景和对象。根据维度指标及推测出的内容生成问卷内容。遵循逻辑顺序：按照维度指标从简单到复杂，从不敏感到敏感。

生成的问卷包含 ${question_count ?? "20"} 道题目，题目不要出现重复或无关的问题。

题目选项默认为"非常同意、同意、一般、不同意、非常不同意"，请按照此选项内容生成题目，题目需要合理，且必须可以适配默认选项。

【格式要求】
- 每个维度用一道 [段落说明] 题作为维度标题
- 该维度下的题目使用 [量表题] 标签
- 同一维度的题目放在一起
- 量表选项为：非常同意、同意、一般、不同意、非常不同意
...
题目选项默认为"非常满意、满意、一般、不满意、非常不满意"，请按照此选项内容生成题目，题目需要合理，且必须可以适配默认选项。

【格式要求】
- 每个维度用一道 [段落说明] 题作为维度标题
- 该维度下的题目使用 [量表题] 标签
- 同一维度的题目放在一起
- 量表选项为：非常满意、满意、一般、不满意、非常不满意
...
2. 题目���述清晰、无歧义，避免文化或性别偏见
3. 简单题侧重基础事实，中等题涉及分析，困难题要求综合应用

题型和数量要求：单选题 ${single_count ?? "10"} 题，多选题 ${multi_count ?? "5"} 题，判断题 ${truefalse_count ?? "5"} 题，填空题 ${fillin_count ?? "5"} 题。必须严格按照各题型对应的数量生成题目。
...
题型和数量要求：单选题 ${single_count ?? "10"} 题，多选题 ${multi_count ?? "5"} 题，判断题 ${truefalse_count ?? "5"} 题，填空题 ${fillin_count ?? "5"} 题。必须严格按照各题型对应的数量生成题目。${DSL_FORMAT_INSTRUCTIONS}
