...
export function registerAnalysisPrompts(server: McpServer): void {
  ...
  server.prompt(
    "nps-analysis",
    "NPS净推荐值完整分析：计算NPS得分、分类分布、趋势对比与改进建议",
    {
...
5. 数据汇总表格`,
  ...
  server.prompt(
    "csat-analysis",
    "CSAT客户满意度分析：计算满意度得分、驱动因素分析与改进优先级",
    {
      survey_id: z.string().describe("问卷编号 (vid)"),
...
- 分隔符 $ 分隔题号和答案
- 分隔符 | 分隔多选答案
- 排序题使用英文逗号分隔选项序号（如 5$2,3,1 表示第5题排序为选项2第1名、选项3第2名、选项1第3名）
- 矩阵/比重/多项填空题使用 题号_子题序号 格式（如 6_1$1}6_2$3 表示第6题矩阵子题1选1、子题2选3）
- 多级下拉使用 - 连接各级选项（如 10$中国-湖南）
...
5. 原始数据汇总`,
  ...
  server.prompt(
    "sentiment-analysis",
    "开放题情感与主题分析：对填空题文本进行情感倾向和关键主题提取",
    {
      survey_id: z.string().describe("问卷编号 (vid)"),
