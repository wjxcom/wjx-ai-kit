...
describe("queryContacts", () => {
  it("should POST with JSON content type", async () => {
    ...
    await queryContacts({ corpid: "test-corp", uid: "user1" }, credentials, fetch, "1700000000");
  ...
  it("should include corpid and uid in request body", async () => {
    const fetch = mockFetch({ result: true, data: {} });
    await queryContacts({ corpid: "my-corp", uid: "alice@example.com" }, credentials, fetch, "100");
  ...
  it("should NOT include username in request body", async () => {
    const fetch = mockFetch({ result: true, data: {} });
    await queryContacts({ corpid: "test-corp", uid: "user1" }, credentials, fetch, "100");
  ...
  it("should throw on HTTP error status", async () => {
    await assert.rejects(
      () => queryContacts({ corpid: "test-corp", uid: "user1" }, credentials, mockFetch("err", 500), "100"),
  ...
  it("should fall back to WJX_CORP_ID env when corpid not provided", async () => {
    ...
    try {
      const fetch = mockFetch({ result: true, data: {} });
      await queryContacts({ uid: "user1" }, credentials, fetch, "100");
  ...
  it("should throw when corpid not provided and WJX_CORP_ID not set", async () => {
    ...
    try {
      await assert.rejects(
        () => queryContacts({ uid: "user1" }, credentials, mockFetch({ result: true }), "100"),
        /corpid is required/,
      );
    } finally {
      if (saved !== undefined) process.env.WJX_CORP_ID = saved;
...
describe("addContacts", () => {
  ...
  it("should POST with JSON content type", async () => {
    const fetch = mockFetch({ result: true, data: {} });
    await addContacts({ corpid: "test-corp", users: usersJson }, credentials, fetch, "1700000000");
  ...
  it("should include corpid and users in request body", async () => {
    const fetch = mockFetch({ result: true, data: {} });
    await addContacts({ corpid: "my-corp", users: usersJson }, credentials, fetch, "100");
  ...
  it("should NOT include username or members in request body", async () => {
    const fetch = mockFetch({ result: true, data: {} });
    await addContacts({ corpid: "test-corp", users: usersJson }, credentials, fetch, "100");
  ...
  it("should NOT retry on 500 (maxRetries=0 for write)", async () => {
    ...
    await assert.rejects(
      () => addContacts({ corpid: "test-corp", users: usersJson }, credentials, fetch, "100"),
      /WJX API request failed with 500/,
    );
  ...
  it("should fall back to WJX_CORP_ID env when corpid not provided", async () => {
    ...
    try {
      const fetch = mockFetch({ result: true, data: {} });
      await addContacts({ users: usersJson }, credentials, fetch, "100");
...
describe("deleteContacts", () => {
  ...
  it("should POST with JSON content type", async () => {
    const fetch = mockFetch({ result: true, data: {} });
    await deleteContacts({ corpid: "test-corp", uids: uidsJson }, credentials, fetch, "1700000000");
  ...
  it("should include corpid and uids in request body", async () => {
    const fetch = mockFetch({ result: true, data: {} });
    await deleteContacts({ corpid: "my-corp", uids: uidsJson }, credentials, fetch, "100");
  ...
  it("should NOT include username, operation, or members in request body", async () => {
    const fetch = mockFetch({ result: true, data: {} });
    await deleteContacts({ corpid: "test-corp", uids: uidsJson }, credentials, fetch, "100");

    const body = JSON.parse(fetch.captured().init.body);
    assert.equal("username" in body, false, "username should not appear in request body");
    assert.equal("operation" in body, false, "operation should not appear in request body");
    assert.equal("members" in body, false, "members should not appear in request body");
  });
  ...
  it("should NOT retry on 500 (maxRetries=0 for write)", async () => {
    ...
    await assert.rejects(
      () => deleteContacts({ corpid: "test-corp", uids: uidsJson }, credentials, fetch, "100"),
      /WJX API request failed with 500/,
    );
    assert.equal(callCount, 1, "deleteContacts should make exactly 1 request (no retries)");
  });
  ...
  it("should throw on HTTP error status", async () => {
    await assert.rejects(
      () => deleteContacts({ corpid: "test-corp", uids: uidsJson }, credentials, mockFetch("err", 500), "100"),
      /WJX API request failed with 500/,
    );
  });
  ...
  it("should fall back to WJX_CORP_ID env when corpid not provided", async () => {
    ...
    try {
      const fetch = mockFetch({ result: true, data: {} });
      await deleteContacts({ uids: uidsJson }, credentials, fetch, "100");
...
describe("queryContacts retry behavior", () => {
  it("should retry on 500 (default maxRetries=2)", async () => {
    ...
    const result = await queryContacts({ corpid: "test-corp", uid: "user1" }, credentials, fetch, "100");
  ...
  it("should throw on 404 without retry", async () => {
    ...
    await assert.rejects(
      () => queryContacts({ corpid: "test-corp", uid: "user1" }, credentials, fetch, "100"),
...
describe("sign consistency", () => {
  it("queryContacts should produce different signs for different timestamps", async () => {
    ...
    await queryContacts({ corpid: "test-corp", uid: "user1" }, credentials, fetch1, "100");
    await queryContacts({ corpid: "test-corp", uid: "user1" }, credentials, fetch2, "200");
  ...
  it("addContacts should produce different signs for different appKeys", async () => {
    ...
    const users = JSON.stringify([{ name: "Test" }]);
    const fetch1 = mockFetch({ result: true, data: {} });
    const fetch2 = mockFetch({ result: true, data: {} });

    await addContacts({ corpid: "test-corp", users }, cred1, fetch1, "100");
    await addContacts({ corpid: "test-corp", users }, cred2, fetch2, "100");
